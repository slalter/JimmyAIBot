...
def hello_http(request):
  ...
  name = "unknown"
  #only initiating posts have a name...
  try:
  ...
  try:
    ...
    request = {"From":request.values.get('From'),"Body":request.values.get('Body'), "Name":name}
  try:
    name = request["Name"]
  except:  
    name = "unknown"
  ...
  if 'start'==str(body):
    ...
    blob = bucket.blob("leads/"+str(fromnum)+".json")
    print(name)
    conversation = Conversation(fromnum,1,name)
